...
class CharRNN():
    ...
    def rnn_layer(self):
        ...
        if self.cell_type == 'GRU':
            for i in range(self.num_layers):
                rnn_cell=tf.nn.rnn_cell.GRUCell(num_units=self.rnn_size)
            ...
            for i in range(self.num_layers): 
                rnn_cell=tf.nn.rnn_cell.LSTMCell(num_units=self.rnn_size)
                if self.train_keep_prob < 1:
                    rnn_cell = tf.nn.rnn_cell.DropoutWrapper(rnn_cell, output_keep_prob=self.train_keep_prob)
                    rnn_cells.append(rnn_cell)
        cell = tf.nn.rnn_cell.MultiRNNCell(rnn_cells)#, state_is_tuple=True)
        try: # initialize weights
            self.initial_state
        except:
    ...
    def my_optimizer(self):
        ...
        clipped_g = [(tf.clip_by_value(grad, -self.grad_clip, self.grad_clip), var) for grad, var in gv] # clipp gradients
        self.optimizer= optimizer.apply_gradients(clipped_g)
        #raise NotImplementedError('Please edit this function.')
        ...
        with self.session as sess:
            ...
            for x, y in batches:
                ...
                if (counter % save_every_n == 0):
                    self.saver.save(sess, "checkpoints/i{}_l{}_{}.ckpt".format(counter, self.rnn_size,self.cell_type))# change the name of checkpoints
    ...
    def sample(self, checkpoint, n_samples, vocab_size, vocab_to_ind, ind_to_vocab, prime='You \n'):
        ...
        samples = [vocab_to_ind[c] for c in prime]
        samples = [vocab_to_ind[c] for c in prime]
        samples = np.array([[[vocab_to_ind[c]]] for c in prime])
        ...
        with self.session as sess:
            ...
            for sample in samples:
                feed = {self.inputs: sample,self.keep_prob: self.train_keep_prob,self.initial_state: new_state}
                preds,new_state = sess.run([self.prob_pred,self.final_state], feed_dict=feed)
            c = pick_top_n(preds, vocab_size, top_n=5)
            prime+=ind_to_vocab[c]
            for i in range(n_samples):
                feed = {self.inputs: [[c]],self.keep_prob: self.train_keep_prob,self.initial_state:new_state}
                preds,new_state = sess.run([self.prob_pred,self.final_state], feed_dict=feed)
                c = pick_top_n(preds, vocab_size, top_n=5)
                prime+=ind_to_vocab[c]
            return(prime)
